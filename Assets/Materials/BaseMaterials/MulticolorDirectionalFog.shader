...
Shader "CustomMaterialAnisotropicFog" {
   Properties {
      _ColorMain ("Diffuse Material Color", Color) = (1,1,1,1) 
      _MidColor ("Semishadow Material Color", Color) = (1,1,1,1) 
      _SpecColor ("Specular Material Color", Color) = (1,1,1,1)
      _HorizontalFogColor ("_HorizontalFogColor", Color) = (1,1,1,1) 
      _VerticalFogColor ("_VerticalFogColor", Color) = (1,1,1,1) 
      _Shininess ("Shininess", Float) = 1
      _AOFactor ("Ambient Occlusion Factor", Float) = 1
      _FogScaleZ ("Fog Scale Z", Float) = 10
      _FogOffsetY("Fog Power", Float) = 2
      _FogProportion("_FogProportion", Float) = 1
   }
   SubShader {
    ...
    Pass {   
         ...
         uniform fixed3 _HorizontalFogColor;
         uniform fixed3 _VerticalFogColor;
         uniform half _FogProportion;
         ...
         vertexOutput vert(appdata_full v) 
         {
            ...
            output.fogColorFactor.w = clamp( _FogProportion/ pos, 0,1);
            half height = mul(modelMatrix, v.vertex).y;
            half poss = (height - _FogOffsetY) / _FogScaleZ;//* lerp(_FogPowerZ,1, height / _FogScaleZ);
            output.fogColorFactor.z = clamp (poss, 0.0, 1.0f);
            output.pos = outpos;
         ...
         half4 frag(vertexOutput input) : COLOR
         {
            ...
            half3 finalColor = lerp(_HorizontalFogColor.rgb, tempcol.rgb, input.fogColorFactor.x);
            half3 tempcolll = lerp(_VerticalFogColor.rgb, _HorizontalFogColor.rgb,1.0 -input.fogColorFactor.w);

            finalColor = lerp(tempcolll.rgb, finalColor.rgb, input.fogColorFactor.z);
